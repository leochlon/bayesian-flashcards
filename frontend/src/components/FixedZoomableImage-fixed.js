...
const FixedZoomableImage = ({ src, alt, className, onLoad, onError }) => {
  ...
  // Use a ref to track the current image source to avoid state loops
  const currentSrcRef = useRef(src);
  const timeoutIdRef = useRef(null);
  const isLoadingRef = useRef(false);

  // Single useEffect to handle all image loading logic
  useEffect(() => {
    ...
    const loadImage = (srcToLoad, attempt = 0) => {
      ...
      img.onload = () => {
        ...
        // Only update state if this is still the current source
